...
async def test_get_config_synth_backend_present(client):
    r = await client.get("/config")
    assert "synth_backend" in r.json()["pipeline"]
...
async def test_put_config_updates_synth_backend(client, config_fixture):
    ...
    assert get_r.json()["pipeline"]["synth_backend"] == "pedalboard_instrument"


@pytest.mark.anyio
async def test_put_invalid_config_returns_422(client):
    r = await client.put("/config", json={"pipeline": {"synth_backend": "invalid_backend"}})
